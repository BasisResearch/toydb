//! Verified concrete parser for the control statements — `BEGIN` / `COMMIT` /
//! `ROLLBACK` — the first brick of the statement-structure cutover (see
//! `verus-parser-roundtrip-plan.md`). It is a 1:1 port of `parser.rs`'s
//! `parse_begin`/`parse_commit`/`parse_rollback`, producing the production
//! `ast::Statement` over `super::Token` and returning the position past the
//! consumed tokens (so the statement parser can check for a trailing semicolon /
//! end of input, exactly like the legacy path).
//!
