...
`git push` to `main` or of a non-prefixed branch. The same guard also backs
the opencode plugin (`tool.execute.before`) and the committed git hooks in
`.githooks/` (pre-commit / pre-push, self-provisioned via `core.hooksPath`),
which cover Codex and humans — the same rules apply to every agent (see
`AGENTS.md`). If it blocks you, rename the branch
(`git branch -m <initials>/<topic>`) and retry — do not try to bypass the
hook.
